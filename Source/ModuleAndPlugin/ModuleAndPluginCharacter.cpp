...
void AModuleAndPluginCharacter::BeginPlay()
{
	...
	if (GetWorld())
	{
		...
		// Test 모듈에 있는 CharacterData를 새로 소환
		UCharacterData* MyData = NewObject<UCharacterData>(this, UCharacterData::StaticClass());

		// 방금 소환한 (MyData->)을 들고, 그 안에 들어있는 체력 정보(MaxHP)를 꺼내서 화면에 청록색으로 5초간 띄움
		GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Cyan, FString::Printf(TEXT("CharacterData에서 가져온 캐릭터 맥스 체력: %f"), MyData->MaxHP));
